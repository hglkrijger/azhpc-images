...
# Use the nvidia-driver-pinning metapackage for version pinning.
# This replaces hardcoded -0ubuntu1 suffix which broke with 580.126.20 (-1ubuntu1).
apt install nvidia-driver-pinning-$NVIDIA_GPU_DRIVER_VERSION -y

# Install the NVIDIA driver and related packages (pinning metapackage handles version resolution)
apt install nvidia-dkms-${NVIDIA_GPU_DRIVER_MAJOR_VERSION}-open \
  nvidia-driver-${NVIDIA_GPU_DRIVER_MAJOR_VERSION}-open \
  nvidia-modprobe -y
